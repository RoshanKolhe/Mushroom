...
export default function CultivationQuickEditForm({ currentCultivation, open, onClose, onRefreshCultivations }) {
  const { enqueueSnackbar } = useSnackbar();

  const { filteredUsers, filteredUsersLoading, filteredUsersEmpty, refreshFilterUsers } =
    useGetUsersWithFilter('filter={"where":{"permissions":["Cultivation_user"]}}');
  ...
  const NewCultivationSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      user: currentCultivation?.userId || '',
      cluster: currentCultivation?.clusterId || '',
      totalCultivation: currentCultivation?.totalCultivation || '',
      isActive: currentCultivation?.isActive ? '1' : '0' || '',
    }),
    [currentCultivation]
  );

  const methods = useForm({
    resolver: yupResolver(NewCultivationSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      await axiosInstance.patch(`/Cultivations/${currentCultivation.id}`, inputData);
      // reset();
  ...
  return (
    ...
    >
      <FormProvider methods={methods} onSubmit={onSubmit}>
        ...
        <DialogContent>
          {!currentCultivation.isActive && (
            <Alert variant="outlined" severity="error" sx={{ mb: 3 }}>
              Cultivation is In-Active
            </Alert>
          )}
          ...
          >
            ...
            <RHFTextField name="name" label="Cultivation Name" />
            <RHFSelect fullWidth name="user" label="Cultivation User">
