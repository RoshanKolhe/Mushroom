...
export default function ClusterListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.cluster.edit(id));
    },
    [router]
  );
  ...
  return (
    <>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Manage Clusters"
          links={[
            { name: 'Dashboard', href: paths.dashboard.root },
            { name: 'Manage Clusters', href: paths.dashboard.cluster.list },
            { name: 'List' },
          ]}
          action={
            <>
        ...
        <Card>
          <ClusterTableToolbar filters={filters} onFilters={handleFilters} />
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (cluster) => cluster.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
